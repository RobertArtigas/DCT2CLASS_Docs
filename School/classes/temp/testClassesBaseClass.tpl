...
#INSERT(%SetFamily)
#FOR(%ApplicationTemplate),WHERE(%ApplicationTemplate='GlobaltestClassesBaseClass(testClassesBaseClass)')
  #FOR(%ApplicationTemplateInstance)
    #CONTEXT(%Application,%ApplicationTemplateInstance)
      #INSERT(%ReadClassesPR,'testClassesBaseClass.inc',%pa,%force)
...
#SHEET,HSCROLL
  ...
  #TAB(UPPER(%Application) & '.' & UPPER(%ProgramExtension))
    #BOXED('GlobaltestClassesBaseClass')
      #DISPLAY(''),AT(,,,1)
    ...
    #BOXED('Debugging Global')
      #PROMPT('Disable All Global testClassesBaseClass Features',CHECK),%NoGlobaltestClassesBaseClass,AT(10)
    #ENDBOXED
    #INSERT(%ClarionFamily)
  #ENDTAB
  #TAB('Multi-Dll'),WHERE(%NoGlobaltestClassesBaseClass=0)
    #BOXED('')
...
#ENDSHEET
#!
#AT(%AfterGlobalIncludes),WHERE(%NoGlobaltestClassesBaseClass=0)
  INCLUDE('testClassesBaseClass.inc'),ONCE
  #INSERT(%IncludeAdditionalIncFiles)
#ENDAT
#!
#ATSTART
  #IF(%NoGlobaltestClassesBaseClass=0)
...
#ENDAT
#!
#AT(%DllExportList),WHERE(%ProgramExtension='DLL' AND %RootDLL=1 AND %MultiDll=1 AND %NoGlobaltestClassesBaseClass=0)
#INSERT(%ExportClassesPR,'testClassesBaseClass.inc')
#INSERT(%ExportAdditionalIncFiles)
#ENDAT
#!
#AT(%CustomGlobalDeclarations),WHERE(%NoGlobaltestClassesBaseClass=0)
#INSERT(%Defines,1,'LinkMode','DllMode',%MultiDLL,%RootDll)
#ENDAT
#!
#AT(%mpDefineAll),WHERE(%NoGlobaltestClassesBaseClass=0)
#INSERT(%Defines,2,'LinkMode','DllMode',%MultiDLL,%RootDll)
#ENDAT
#!
#AT(%mpDefineAll7),WHERE(%NoGlobaltestClassesBaseClass=0)
...
#SHEET,HSCROLL
  ...
  #INSERT(%LicenseMe)
  #TAB(UPPER(%Application) & '.' & UPPER(%ProgramExtension))
    #BOXED('LocaltestClassesBaseClass')
      #DISPLAY(''),AT(,,,1)
      #DISPLAY('testClassesBaseClass')
      #DISPLAY('Templates copyright (c) by each individual author.')
      #DISPLAY('E-MAIL: roberto.artigas.dev@gmail.com')
    ...
    #BOXED('Debugging Local')
      #PROMPT('Disable Local testClassesBaseClass Features',CHECK),%NoLocaltestClassesBaseClass,AT(10)
    #ENDBOXED
    #INSERT(%ClarionFamily)
  #ENDTAB
  #TAB('Classes')
    #BOXED('')
      ...
      #PROMPT('&Class Name',@s100),%zClassName,DEFAULT('testClassesBaseClass')
    #ENDBOXED
  #ENDTAB
#ENDSHEET
#!
#ATSTART
  #IF(%NoLocaltestClassesBaseClass=0)
    #INSERT(%AtStartInitialisation)
    #INSERT(%AddObjectPR,%zClassName,%zObjectName,'Local Objects')
  #ENDIF
#ENDAT
#!
#AT(%DataSection),WHERE(%NoLocaltestClassesBaseClass=0),DESCRIPTION('LocaltestClassesBaseClass - Class Definition'),PRIORITY(2700)
#INSERT(%GenerateClassDeclaration,%zClassName,%zObjectName,'Local Objects','RYB Objects')
#ENDAT
#!
#AT(%LocalProcedures),WHERE(%NoLocaltestClassesBaseClass=0),DESCRIPTION('LocaltestClassesBaseClass - Class Definition')
